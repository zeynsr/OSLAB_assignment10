...
class Game(arcade.Window):
    ...
    def on_draw(self):
        if self.snake.score <= 0 or  self.snake.center_x < 0 or self.snake.center_x > SCREENWIDTH or self.snake.center_y < 0 or self.snake.center_y > SCREENHEIGHT:
            arcade.draw_text("Game Over!", (SCREENWIDTH // 4) - 40, SCREENHEIGHT // 2, arcade.color.RED, FONTSIZE*4, width = 400, align = "left")
        else :
            arcade.start_render() 
            self.snake.draw()
            self.food1.draw()
...
class NegtivePoint(arcade.Sprite) :
    def __init__(self) :
        ...
        self.width =16
        self.height=16   
        self.center_x = random.randint(20,SCREENWIDTH-20)  
        self.center_y = random.randint(20, SCREENHEIGHT-20)  
